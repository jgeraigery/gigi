    ...
    void Context::EnsureResourcesCreated(ID3D12Device* device, ID3D12GraphicsCommandList* commandList)
    {
        ...
        // Loaded from: imageb%i.png
        {
            if (!m_internal.texture__loadedTexture_0)
            {
                ...
                m_internal.texture__loadedTexture_0 = DX12Utils::CreateTexture(device, size, desiredNumMips, m_internal.texture__loadedTexture_0_format, m_internal.texture__loadedTexture_0_flags, D3D12_RESOURCE_STATE_COPY_DEST, DX12Utils::ResourceType::Texture3D, (c_debugNames ? L"_loadedTexture_0" : nullptr), Context::LogFn);
